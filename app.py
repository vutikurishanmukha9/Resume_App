...
class ModelManager:
    ...
    def load_models(self):
        ...
        try:
            logger.info("Model loader started")
...
# Start loader in background so server responds quickly
...
@app.route("/ready")
def ready():
    """Readiness endpoint that returns JSON status"""
    return jsonify({"ready": bool(_model_loaded)})
...
if __name__ == '__main__':
    ...
    logger.info("Starting AI Resume Analyzer server...")
    logger.info("Models will load in background. Server will respond immediately.")
